...
A \gls{ch} accumulates data after a given time interval to avoid concurrent events with e.g. a \gls{ch} election. The \gls{ch} will also accumulate data 'X' times before it broadcasts a new \gls{ch} calculation and eventually a \gls{ch} election.
...
At the current approach, there is no timestamp or regulation for when the \gls{ch} was elected. If a node was unavailable during the \gls{ch} election, it will forward its data to the, most likely, old \gls{ch}. However, this isn't a drawback since the node receiving this data will probably forward the accumulated data in the next gathering phase.
