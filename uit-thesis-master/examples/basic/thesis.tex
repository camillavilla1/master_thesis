...
Results show that the system have a steady memory between 60\% and 76\% and CPU usage around 75\%  during execution. Experiments also show that the \gls{ch} receives fewer packets of data than sent packets from nodes in the cluster which indicates that the nodes in the system accumulates data when intended.
...
The result is presented in Figure \ref{fig:memChart}. We can see that the system has between 60\% and 76\% memory usage during execution and the graphs shows that each experiment us approximately the same amount of memory.
