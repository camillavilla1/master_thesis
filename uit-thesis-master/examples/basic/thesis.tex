...
A node will only be able to connect to another node in the network if the node accept the request to be neighbours. Right now will every node be able to connect with their neighbours unless a node is unavailable. There is no restriction in number of neighbours a node can have or that a node receiving a request from a new neighbour must forward the request to the \gls{ch} and let the leader decide weather the new node can connect to the neighbour or not. 
