...
%Partially centralized - supernodes act as local central indexes, some nodes assume a more important role

%hva er veien, ikke hvilken vei..

The technical idea behind the system is that it should be a decentralized unstructured \gls{p2p} system  where nodes connect to nearby nodes and create clusters. The role of all peers in the network is the same except that there would be at least one node which will gather data for further use, also called a \gls{ch}. The technical idea is shown in Figure \ref{fig:idea}.
...
%"Being a cluster head is much more energy intensive than eing a non-cluster head node. if the clulster heads where chosen a priori adn fied through the system lifetime, these nodes would quickly use up their limited energy. Once the CH runs out of energy, it is no longer operational, and all the nodes that belong to the cluster lose communcation ability."

%Gossiping: max number of steps/hops when gossiping to avoid flooding and overhead.. reduce nodes energy level etc..
