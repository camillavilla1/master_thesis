...
\thesisprogramme{INF-3981 Master's Thesis in Computer Science \\
 June 1, 2018}
...
The Arctic Tundra in the far northern hemisphere is one of ecosystems that are most affected by the climate changes in the world today. Five Fram Center\footnote{\url{http://www.framsenteret.no/english}} institutions developed a long-term research project called \glsfirst{coat}. Their goal is to create robust observation systems which enable documentation and understanding of climate change impacts on the Arctic tundra ecosystems.


This thesis describes a prototype of a \glsfirst{wsn} system where nodes in the network creates clusters of Observation Units (\glspl{ou}) to accumulate data. The purpose is to fetch and accumulate data observed by \glspl{ou} for further use and to provide for a more flexible and powerful sensor in the \gls{coat} monitoring of the Arctic Tundra.

We describe a system where nodes discover each other through a range limited broadcast. Together they form clusters. Each cluster elect a \glsfirst{ch} which is responsible for sending out a request for gather and accumulate data from the other nodes in the cluster. The role as \gls{ch} is rotating among the nodes to conserve battery.

Results show that the system have a steady memory usage between 60\% and 76\% and CPU usage around 75\%  during execution. Experiments also show that the \gls{ch} received fewer packets of data compared to sent packets from \glspl{ou} in the cluster which indicates that the \glspl{ou} in the system accumulates data when intended.

The proposed prototype of the system proved capable of electing \glspl{ch} that gathers and accumulates data efficiently. As a prototype, it still has room for improvements such as the availability of nodes in the system, \gls{ch}-elections and multiple \glspl{ch} in each cluster. A future system could further investigate the benefits of having multiple \glspl{ch} and how to gather and accumulate data more efficiently. There is still a need for conducting further work for a real-life environment in the Arctic Tundra.
...
%Data aggregation is an essential operation in \glspl{wsn} and has emerged as a basic approach in order to reduce the number of transmissions of sensor nodes \cite{dataAgg}.
...
If there is a \gls{ch} in the cluster already and a new election should be proposed, a \gls{ch}-calculation request is sent from the \gls{ch}. Nodes receiving this request will calculate their \gls{ch}-election number. %This is explained further in Section xx.
...
This request forwards a nodes data to the next node in the path to the \gls{ch}. If the node receiving this requests hasn't sent its data to the \gls{ch} of the cluster, the data will be accumulated with the received data and then forwarded to the next node.
...
\caption{Figure show how a \gls{ch} gossips a GET-data request and how  nodes sends their data to the \gls{ch} through the nodes in the path.}
...
\subsection{Cluster Head Data Accumulation}
When a \gls{ch} starts a gathering of data, the node will chose a value between one and six which indicates how many times it should gather data before it eventually sends out a new \gls{ch}-election. We chose to implement the \glspl{ch} to accumulate data between one and six times because finding the exact number of how many times a \gls{ch} should accumulate data wasn't our primary focus.
...
%The environment for evaluating our system is built with gopsutil, a psutil for Golang \cite{golangPsutil}.
...
The log-file contains data for each time interval (500 millisecond) where  one row is one node, identified by its process ID, and one column is one time interval during execution. We calculate each column in the log-file to obtain an average value because of the huge amount of data. The graphs displayed contains the average values for each time interval (500 millisecond) with a standard deviation of all values.
...
%\textbf{Fjerne?? Since each experiment is launched independently, the clusters in the network may appear different in each experiment causing nodes to have more or less neighbour nodes and a longer or shorter path to the \gls{ch} than in another experiment.}
...
%The memory usage is essential to examine because it ... to be relatively low to be able to make the device function as normal in a real-life environment/scenario.
...
%The number of sends are counted every time the node sends a request to a neighbour, like discovering other nodes, connecting to other nodes, \gls{ch}-election or sending and accumulating data to \gls{ch}.

We measure the number of sends independently for each node every 500 milliseconds during execution every time the node sends a request to a neighbour, like discovering other nodes, connecting to other nodes, \gls{ch}-election or accumulating and sending data to the \gls{ch}.
...
%\textbf{JM: en ch er vel også en vanlig ou. betyr dette at vi overser noe, eller er det bare acc-comm du er interessert i na?}

% Besides measuring number of sends to neighbours, we measured number of sends to a \gls{ch} during execution. 
%The number of sends are only counted when there is a accumulation request to send data to a \gls{ch}.
...
We measure how many times nodes have been \glspl{ch} at each time interval to examine if how many \glspl{ch} there is at each time interval. The \gls{ch} is only counted when it's done gathering all the data.
...
The CPU percentage is stable on around 75\% during the execution for each experiment. We believe that the reason there are no peaks during execution, is because of Golangs\footnote{\url{https://golang.org/}} built-in language features and ability to utilize multicore and networked machines.
...
From \autoref{fig:cpuChart}, we can see that each implementation uses less CPU usage in the start and end of the execution. This is because in the beginning, nodes take time to start and to find neighbours to connect to. Then, the CPU usage will increase because nodes that have found nearby neighbours to connect to and they will start \gls{ch}-elections.
...
\begin{figure*} [ht]
        ...
        \begin{subfigure}[b]{0.475\textwidth}
            \centering
            \includegraphics[width=\textwidth]{CPU_stdev_2.png}
            \caption[Network2]%
            {{\small Network 1}}    
            \label{fig:cpufig1}
        \end{subfigure}
        ...
        \begin{subfigure}[b]{0.475\textwidth}  
            \centering 
            \includegraphics[width=\textwidth]{CPU_stdev_4.png}
            \caption[]%
            {{\small Network 2}}    
            \label{fig:cpufig2}
        \end{subfigure}
...
%The open connections are closed after each exchange

There is also reason to think that these wave like patterns occurs at different time intervals is because of the different implementations accumulate data and elect \glspl{ch} at different time intervals. Each cluster of nodes may also get out of sync with the other clusters resulting in that accumulation of data and \gls{ch}-elections occurs at different iterations. This may smooth out the graphs eventually, as seen in \autoref{fig:connfig3}
...
\begin{figure*}
        ...
        \begin{subfigure}[b]{0.475\textwidth}
            \centering
            \includegraphics[width=\textwidth]{numSends_stdev_2.png}
            \caption[Network2]%
            {{\small Network 1}}    
            \label{fig:sendfig1}
        \end{subfigure}
        ...
        \begin{subfigure}[b]{0.475\textwidth}  
            \centering 
            \includegraphics[width=\textwidth]{numSends_stdev_4.png}
            \caption[]%
            {{\small Network 2}}    
            \label{fig:sendfig2}
        \end{subfigure}
...
%\caption{Figure shows average number of sends to \glspl{ch} with different accumulation intervals.}
...
\begin{figure*}
        ...
        \begin{subfigure}[b]{0.475\textwidth}
            \centering
            \includegraphics[width=\textwidth]{numSendsCh_stdev_2.png}
            \caption[Network2]%
            {{\small Network 1}}    
            \label{fig:sendchfig1}
        \end{subfigure}
        \hfill
        \begin{subfigure}[b]{0.475\textwidth}  
            \centering 
            \includegraphics[width=\textwidth]{numSendsCh_stdev_4.png}
            \caption[]%
            {{\small Network 2}}    
            \label{fig:sendchfig2}
        \end{subfigure}
        \vskip\baselineskip
        \begin{subfigure}[b]{0.475\textwidth}   
            \centering 
...
There is reason to think that all the graphs, except the graph of \gls{ch}-count after a random number of accumulations, have flat graphs during execution because all the \gls{ch}-counts happens approximately at the same time interval and don't change until the next \gls{ch}-election.
...
%\textit{Blue graph increase to almost double because of \gls{ch}-elections in cluster. Since only two gatherings of data, more time to elect new \glspl{ch}. Previous \gls{ch} elected as \gls{ch} again, causing the average value to increase.. hvorfor oker ant CHs? Partisjonering?}
...
%\textit{Orange graph have a little increase, some clusters have elected new \gls{ch}. Use longer time to accumulate data since it's gathering data six times instead of two or four..}
...
\caption{Figure shows the number of \glspl{ch} for 100 nodes per network with different accumulation intervals.}
\label{fig:chCountChart}
\end{figure}
...
\begin{figure*}  [ht]
        ...
        \begin{subfigure}[b]{0.475\textwidth}
            \centering
            \includegraphics[width=\textwidth]{RecvPkt_stdev_2.png}
            \caption[Network2]%
            {{\small Network 1}}    
            \label{fig:recvfig1}
        \end{subfigure}
        ...
        \begin{subfigure}[b]{0.475\textwidth}  
            \centering 
            \includegraphics[width=\textwidth]{RecvPkt_stdev_4.png}
            \caption[]%
            {{\small Network 2}}    
            \label{fig:recvfig2}
        \end{subfigure}
        ...
        \begin{subfigure}[b]{0.475\textwidth}   
            \centering 
            \includegraphics[width=\textwidth]{RecvPkt_stdev_6.png}
            \caption[]%
            {{\small Network 3}}    
            \label{fig:recvfig3}
        \end{subfigure}
        ...
        \begin{subfigure}[b]{0.475\textwidth}   
            ...
            \label{fig:recvfig4}
        \end{subfigure}
        \caption[Figures show average number of \glspl{ch} for 100 nodes per network with different accumulation intervals with standard deviation.]
        {\small Figures show average number of \glspl{ch} (dark colors) for 100 nodes per network with different accumulation intervals with standard deviation (lighter colors).} 
        \label{fig:recPktChart}
    \end{figure*}
...
%\textbf{JM: wake-up call: har du tenkt noe pa hvordan dette kunne gjores med sovende ouer i feltet? time slots for nar de er vakne rimer med tanken om  ping ved bestemte tidspunkt eller hendelser.}
...
%\textbf{JM: husker ikke i farten: har du tatt for deg en node eller til og med en CH som blir borte eks: tom for strom, krasjer eller noe skjer som odelgger den (flom, dyr, etc..)}
...
%\textbf{JM: Tja.. samtidig slipper du long range conn som ogsa kan vare ustabilt. Tradeoffs som du sier}
