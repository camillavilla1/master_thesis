...
Note to self: Every node have batterytime.. eventually dies, ch battery decrease more than regular node..
...
The current approach is similar to \gls{leach} in means of different phases for electing a new \gls{ch} and accumulate data. We chose this solution because of concurrency issues when these two events happen at the same time, as mentioned in Section \ref{sec:conc_events}. 
...
A solution to this is to introduce multiple leaders to load balance work and may also provide shorter path for some nodes. The question is then which \gls{ch} should a node choose to be its \gls{ch} if there are multiple. Most likely will the \gls{ch} election be based on several sub-factors described in Section \ref{ssec:ch_election}. If we assume that all nodes that are qualified/efficient/capable to become \gls{ch}s are equivalent/have the same "...", the node should probably choose the \gls{ch} with the shortest path to avoid flooding the network with requests and to avoid draining the limited energy on more nodes than needed/necessary.
...
The current approach provides an eventual consistency when electing a new \gls{ch} as mentioned before. This may raise some issues when a data gathering occurs when an election is ongoing due to rapidly change of \gls{ch}s and paths to \gls{ch} before all nodes have an consistent \gls{ch} election.
