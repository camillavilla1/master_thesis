...
!!! Fa frem at hver node er en unik prosess...


The system is implemented in the open source programming language GO 1.9.3\footnote{\url{https://golang.org/}}. Each node is launched as a unique process and they communicate with each other by Golangs HTTP Server which listens on the \gls{tcp} network. This was because it's developed for making concurrent mechanisms easy and to utilize multicore and networked machines, and offers multiple different libraries.
...
It is not suitable to have nodes deployed in the Arctic Tundra at such an early development. This implementation simulates a real-life environment where nodes can communicate with each other through the \gls{tcp} network. Each node is assigned \textit{x,y} coordinates within a network grid size and a broadcast width.
...
Otto: uklart why this statement is true? Why 6??

\textbf{A \gls{ch} accumulates data after a given time interval to avoid concurrent events with e.g. a \gls{ch} election.} 
When a \gls{ch} starts a gathering of data, the node will chose a value between one and six which indicates how many times it should gather data before it eventual sends out a new \gls{ch} election.
...
\textit{Otto: Forklar litt mer om fasene...}

\textit{JM: kan du utvide dette til å virke uten failed nodes?}
...
Presently, the \gls{ch} election is based on which node has the highest score between 0 and 1. This \textbf{intellectually} simplified does not take in account many other realistic aspects which would improve the \gls{ch} election. To improve \textbf{HOW?????} this approach and make it more realistic, we could have used several sub-factors listed below:
