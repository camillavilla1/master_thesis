...
The idea behind the system is that it should be a decentralized unstructured \gls{p2p} system  where nodes connect to nearby nodes and create clusters. The role of all peers in the network is the same except at each cluster, where there would be at least one \gls{ch} which will gather data for further use. The technical idea is shown in Figure \ref{fig:idea}.
