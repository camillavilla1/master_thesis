...
This thesis describes a prototype of a \gls{wsn} system with the purpose of fetching and accumulating data observed by \glspl{ou} for further use.
...
\section{Motivation}
This dissertation will develop an approach to let \gls{ou} observe each other and gradually accumulate data to \glspl{ou} being a \gls{dao}-store. There can be multiple \glspl{dao}-stores depending on user needs. The purpose is to fetch and accumulate data observed by \gls{ou}s for further use.
...
Figure \ref{fig:sentvsrecvChart} show the number of sends from nodes to neighbours, sends from nodes to \gls{ch}s and number of times \gls{ch}s have received data. As we can see, are 
...
Having multiple \glspl{ch} that collects data can make 

Replication of node data is important if a node fails and we need to access the nodes data. However, replication of data also require more memory but sensor nodes have limited memory. 


%if one node fails, you can find data at another node
%Replication of data can be 
...
Our experiments showed that the system have a stable CPU usage. Memory usage.. We can also see that the number of receiving packets of accumulated data is lower than sent packets with accumulated data.
...
\begin{itemize}
\item \textbf{\gls{ch} election:}\gls{ch} election/calculation, multiple leaders to load balance and replication(?)
\item \textbf{Access to BS:}
\item \textbf{Multiple \glspl{ch}:} Multiple Super \gls{ou}s that accumulates data..
